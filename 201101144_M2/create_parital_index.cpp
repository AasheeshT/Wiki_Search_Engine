...
void insert()
{
	...
	{
		ind[temp][docId].category[ state.top() ] += 1; 
		if( ind[temp][docId].category[ state.top() ] > maxCount[docId].category[ state.top() ] )
		{
			maxCount[docId].category[state.top()] = ind[temp][docId].category[ state.top()];
		}
	}
}
void buildInd()
{
	...
	/*for( itmax = maxCount.begin(); itmax != maxCount.end() ; itmax++)
	{
		...
		for(int i=0;i<5;i++)
			fprintf(fdmax," %d", itmax->second.category[i]);
		fprintf(fdmax, "\n");
	}*/
	fclose( fd );
	//fclose( fdmax );
...
int main(int argc , char* argv[])
{
	...
	outfile = "./201101144_indexes/" + outfile + ".index";
